...
def extract_data(file_name):
    ...
    with open(file_name, 'r') as file_data:
        """ 
        Convert characters to lowercase (avoid mismatch) 
        and remove spaces from begin and the end of the line 
        """
        for line in file_data:
            ...
            # build phones list
            """ 
            Go over each word in line, add them as keys if not in the dictionary, 
            if present, count its occurrences 
            """
            for word in _line:
                if len(word) > 2:
...
def call_text_review_code(my_article):
    word_occur = extract_data(my_article)
    ...
    # How many unique words are there?
    unique_words = get_unique_words(my_article)
    ...
    phone_list = get_phone_numer(my_article)
    if not phone_list:
